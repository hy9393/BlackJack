...
var currentWager = 0;
var totalChipBalance = localStorage.getItem('blackjackChips') || 1000;
var currentChipBalance = totalChipBalance;
...
var resetChipButton = $("#reset-chip-button");
...
var newResetChipButton = $(".new-reset-chip-button"); 
...
function updateVisibleChipBalances() {
	...
	localStorage.setItem('blackjackChips', totalChipBalance);
}
...
function selectWager(amount){
	if (currentChipBalance - amount >= 0) {
		currentWager += amount;
...
// Called when player clicks the reset button for betting chip
function resetWager() {
	currentWager = 0;
	currentChipBalance = totalChipBalance;
	updateVisibleChipBalances();
}
